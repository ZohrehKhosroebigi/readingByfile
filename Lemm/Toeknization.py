...
class Tokenization():
    ...
    def token(self,doc_name):
        ...
        doc_stop_words=open('logs\stopwords_news.dict_NEW.txt', 'r', encoding='utf8')
        stop_words=[]
        ...
        for line in doc:
            ...
            print(token_query)
            print(stop_words)
            dif_toekn=list(set(token_query) - set(stop_words))
            print("token_query----"+str(dif_toekn))
            fhandw.write(str(dif_toekn))
            fhanddiffstop.write(str(list(set(stop_words) - set(token_query))))
        return self.name
